...
class AppDatabase extends _$AppDatabase {
  AppDatabase() : super(FlutterQueryExecutor.inDatabaseFolder(path: 'moorList.sqlite', logStatements: true));
  ...
  Future<List<Task>> getTasks(String title) => (select(tasks)..where((task) => task.title.equals(title))).get();

  Future<int> addTask(Task task) => into(tasks).insert(task);

  Future<int> deleteTitle(String title) => (delete(tasks)..where((task) => task.title.equals(title))).go();
